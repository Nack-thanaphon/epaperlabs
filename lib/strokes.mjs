...
export function undo(store, redoStack = []) {
  if (store.length === 0) return false;
  redoStack.push(store.pop());
  return true;
}

/** Restore the most recently undone stroke. Returns true if something was restored. */
export function redo(store, redoStack = []) {
  if (redoStack.length === 0) return false;
  store.push(redoStack.pop());
  return true;
}
